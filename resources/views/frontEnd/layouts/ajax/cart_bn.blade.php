...
<table class="cart_table table table-bordered table-striped text-center mb-0">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-left">
                 <a style="font-size: 14px;" href="{{route('product',$value->options->slug)}}"><img src="{{asset($value->options->image)}}" height="30" width="30"> {{Str::limit($value->name,20)}} <br> {{$value->options->product_color ?? ''}} <br> {{$value->options->product_size ?? ''}}</a>
            </td>
