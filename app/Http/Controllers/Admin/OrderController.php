...
class OrderController extends Controller
{
    ...
    public function index($slug, Request $request)
    {
        ...
        $show_total = Order::where('order_status', 6)->sum('amount');
        $total_amount = $show_data->sum('amount');
        ...
        return view('backEnd.order.index', compact('show_data', 'order_status', 'users', 'steadfast', 'pathaostore', 'pathaocities', 'total_amount', 'show_total'));
    }
    ...
    public function order_pathao(Request $request)
    {
        ...
        if ($pathao_info) {
            $response = Http::withHeaders([
                ...
                'recipient_address' =>  $order->shipping->district . ' ' . $order->shipping->area . ' ' . $order->shipping->address,
    ...
    {
        $data = Order::where(['invoice_id' => $invoice_id])->select('id', 'invoice_id', 'order_status', 'order_type', 'advance')->with('orderdetails', 'shipping')->first();

        $districts = District::distinct()->select('district')->orderBy('district', 'asc')->get();
        $areas = District::where('district', $data->shipping->district)->get();
        return view('backEnd.order.process', compact('data', 'districts', 'areas'));
    }
    ...
    public function order_process(Request $request)
    {
        ...
        if ($request->status == 6) {
            ...
            // refer commision
            $netprofit = $order->amount - ($order->shipping_charge + $order->purchase_amount);
            $customer = Customer::select('id', 'refferal_1')->find($order->customer_id);

            // level1 profit share
            // $level1_customer = Customer::where(['id'=>$order->reseller_id,'status'=>'active'])->first();
            // if($level1_customer){

            //     $generaldata = GeneralSetting::first();
            //     $amount    = round(($netprofit*$generaldata->commission)/100,2);
            ...
            //     $profit_save->profit      = round(($netprofit*$generaldata->commission)/100,2);
            //     $profit_save->level       = 1;
            //     $profit_save->type        = $generaldata->id;
            //     $profit_save->save();
            // }
            ...
            if ($order->reseller_id) {
                $balance_update = Customer::select('id', 'balance','earning')->find($order->reseller_id);
                $balance_update->balance += $order->commission;
                $balance_update->earning += $order->commission;
                $balance_update->save();


                $transaction_data = Transaction::create([
                    'user_id'     => $balance_update->id,
                    'user_type'   => 'customer',
        ...
        if ($request->status == 8) {
            ...
            if($order->seller_id == 1 &&  $order->reseller_id == NULL){
                $expense = new Expense();
                $expense->name = 'Order return - ' . $order->invoice_id;
                $expense->expense_cat_id = 4;
                $expense->amount = $return_amount + $amount;
                $expense->note = 'Order return - ' . $order->invoice_id;
                $expense->date = Carbon::now();
                $expense->status = 1;
                // $expense->save();
            } elseif($order->seller_id == 1){
                $expense = new Expense();
                $expense->name = 'Order return - ' . $order->invoice_id;
                $expense->expense_cat_id = 4;
                $expense->amount = $return_amount;
                $expense->note = 'Order return - ' . $order->invoice_id;
                $expense->date = Carbon::now();
                $expense->status = 1;
                // $expense->save();
            } elseif($order->seller_id != 1 &&  $order->reseller_id == NULL){
                $seller = Seller::where(['id' => $order->seller_id, 'status' => 'active'])->first();
                $seller->balance -= $return_amount + $amount;
                return $seller;
                // $seller->save();
            }elseif($order->seller_id != 1 &&  $order->reseller_id){
                $seller = Seller::where(['id' => $order->seller_id, 'status' => 'active'])->first();
                $seller->balance -= $return_amount;
                // $seller->save();
            }
        ...
        $shippingfee = District::find($request->area);

        if ($order->shipping_charge > $shippingfee->shippingfee) {
            $total = $order->amount + ($shippingfee->shippingfee - $order->shipping_charge);
            $order->shipping_charge = $shippingfee->shippingfee;
            $order->amount = $total;
            $order->save();
        } else {
            $total = $order->amount + ($shippingfee->shippingfee - $order->shipping_charge);
        ...
        $shipping_update->district = $shippingfee->district;
        $shipping_update->area = $shippingfee->area_name;
    ...
    public function order_status(Request $request)
    {
        ...
        if ($request->order_status == 8) {
            ...
            foreach ($orders as $order) {
                ...
                // level1 profit share
                $level1_customer = Customer::where(['id' => $order->reseller_id, 'status' => 'active'])->first();
                // return $level1_customer;
                if ($level1_customer) {
                    $level1_customer->balance -= ($amount / 2);
                    $level1_customer->save();
                }
            }
        }
    ...
    public function order_create()
    {
        ...
        return view('backEnd.order.create', compact('products', 'cartinfo', 'districts'));
    }
    ...
    public function order_store(Request $request)
    {
        ...
        if ($request->guest_customer) {
            ...
            $area = District::where('pos', 1)->first();
            $name = $customer->name;
        ...
        $shipping_area  = District::where('id', $request->upazila)->first();
        ...
        $shipping->area        =   $request->upazila ? $shipping_area->area_name : 'Free Shipping';
        $shipping->save();
    ...
    {
        $shipping = District::where('id', $request->id)->first()->shippingfee;
    ...
    public function order_edit($invoice_id)
    {
        ...
        $cartinfo  = Cart::instance('pos_shopping')->content();
        return view('backEnd.order.edit', compact('products', 'cartinfo', 'shippinginfo', 'order', 'data', 'districts', 'areas'));
    }
    ...
    public function order_update(Request $request)
    {
        ...
        $shipping_area = District::where('area_name', $request->upazila)->first();
    ...
    public function reseller_cash_report(Request $request)
    {
        $data = Customer::where('seller_type', 1)->where('balance', '!=', 0)->paginate(50);
        return view('backEnd.reports.reseller_cash', compact('data'));
    }
    public function loss_profit(Request $request)
    {
        ...
        return view('backEnd.reports.loss_profit', compact('total_expense', 'total_purchase', 'total_sales', 'total_income'));
    }
}
